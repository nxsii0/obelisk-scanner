...
def _agent_log(*, hypothesis_id: str, location: str, message: str, data: dict) -> None:
    ...
    try:
        ...
        with open("debug-e9e8d2.log", "a", encoding="utf-8") as f:
...
def _query_package(pkg: dict, *, policy: HttpPolicy) -> tuple[dict, list[dict]]:
    _agent_log(
        hypothesis_id="H2",
        location="obeliskscan/cli/run.py:_query_package:entry",
        message="Querying package",
        data={"name": pkg.get("name"), "version": pkg.get("version"), "has_shodan_cves": ("shodan_cves" in pkg)},
    )
    if "shodan_cves" in pkg:
        ...
        _agent_log(
            hypothesis_id="H4",
            location="obeliskscan/cli/run.py:_query_package:shodan_done",
            message="Shodan CVE enrichment complete",
            data={"name": pkg.get("name"), "count": len(all_cves)},
        )
        return pkg, all_cves
...
def _export_reports(results: list[tuple[dict, list[dict]]], packages: list[dict], duration: float, output_dir: Path, target_name: str, formats: str = "all"):
    ...
    _agent_log(
        hypothesis_id="H3",
        location="obeliskscan/cli/run.py:_export_reports:entry",
        message="Starting export",
        data={"output_dir": str(output_dir), "target_name": target_name, "formats": formats},
    )
...
def run_scan(args) -> int:
    _agent_log(
        hypothesis_id="H1",
        location="obeliskscan/cli/run.py:run_scan:entry",
        message="run_scan called",
        data={
            "file": getattr(args, "file", None),
            "dir": getattr(args, "dir", None),
            "package": getattr(args, "package", None),
            "target": getattr(args, "target", None),
            "target_ports": getattr(args, "target_ports", None),
            "format": getattr(args, "format", None),
            "output_dir": getattr(args, "output_dir", None),
            "timeout": getattr(args, "timeout", None),
            "ci": getattr(args, "ci", None),
            "no_export": getattr(args, "no_export", None),
        },
    )
...
def main() -> None:
    ...
    _agent_log(
        hypothesis_id="H1",
        location="obeliskscan/cli/run.py:main:parsed_args",
        message="Parsed CLI args",
        data={"command": getattr(args, "command", None)},
    )
